...
export default function TrainerSignupPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setForm({ fullName: '', phone: '', email: '', governmentId: '', profilePhotoUrl: '', experience: '' });
    } catch (submitError) {
    ...
    <main className="pwa-shell auth-shell public-shell">
      ...
      <section className="login-hero public-hero">
        <div className="brand-cluster">
          <BrandLogo />
          <div>
            <p className="eyebrow">Trainer Request</p>
            <h1>Apply as a Trainer</h1>
            <p className="subcopy">Send your request in a few simple steps. The owner will review it before account access is given.</p>
          </div>
        </div>
        ...
        <article className="panel public-panel">
          ...
          <form className="stack-form" onSubmit={submit}>
            <div className="form-section-grid">
              ...
              <label>
                Experience
                <textarea value={form.experience} onChange={(event) => setForm((prev) => ({ ...prev, experience: event.target.value }))} placeholder="Optional training experience" />
              </label>
            </div>

            <PhotoUpload label="Profile Photo" value={form.profilePhotoUrl} onChange={(next) => setForm((prev) => ({ ...prev, profilePhotoUrl: next }))} />
          ...
          <div className="auth-links align-left">
            ...
            <Link href="/signup/client" className="ghost-button">I am a Client</Link>
          </div>
        </article>
      </section>
    </main>
